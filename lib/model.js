...
var Model = module.exports = Backbone.Model.extend({
    ...
    parse: function( resp, options ){
        ...
        var modelDef = this.constructor.modelDef;

        // retrieve property descriptions from the schema
        var schemaProperties = modelDef.retrieveSchemaProperties( _.keys(resp) );
        ...
        for( var name in resp ){
            ...
            // then resolve it 
            if( schemaValue ){
                resp[name] = modelDef.registry.resolve( resp[name], schemaValue );
            }
        }
