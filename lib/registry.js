    ...
    _mapEntitySetToProcessor: function( entitySet, processor ){
        ...
        if( processor.entityFilter ){
            ...
            if( this._entityCollections[ hash ] ){
                view = this._entityCollections[ hash ];
            } else {
                view = EntitySet.createView( entitySet, filter );
                this._entityCollections[ hash ] = view;
    ...
    */
    updateSync: function( timeMs, options ){
        ...
        this._entitySetProcessors.each( function(processorRecord){
            ...
            var view = processorRecord.get('view') || processorRecord.get('entitySet');
            var entityArray = view.models;

            // if( debug ){ log.debug('executing processor ' + processor.name + ' ' + processor.get('priority') + ' with ' + view.hash() + ' ' + entityArray.length + ' models'); }
            ...
            if( view.isModified ){
                // log.debug('processor view ' + processor.name + ' modified');
                view.applyEvents();
            } else {
                // log.debug('processor view ' + processor.name + ' not modified');
            }
        });
