...
_.extend(Registry.prototype, Backbone.Events, {
    ...
    initialize: function(options){
        ...
        this._entitySetProcessors = createProcessorCollection();// {};    

        // a map of hashes to entityCollections
    ...
    createEntityFilter: function( entityFilter, options ){
        ...
        if( _.isArray( args[0] ) ){
            ...
            args = _.map( args, function(filterDef){
                var filterType = filterDef[0];
                var componentIds = self.getIId.apply( self, filterDef.slice(1) );
                return [filterType].concat( componentIds );
            });

            result = EntityFilter.create.apply( null, args );

        } else if( EntityFilter.isEntityFilter(entityFilter) ) {
            result = entityFilter;
        } else {
            filterType = args[0];
            componentIds = this.getIId.apply( this, args.slice(1) );

            result = EntityFilter.create( [filterType].concat( componentIds ) );
        }
    ...
    */
    ...
    /**
        ...
        a collection can be generated in that case.
        The given entitySet may also specify that it is not optimal for
        an update, so in that case a collection will also be generated.

        Entity collections are stored according to their hash, so that
        ...
        - if the hash doesn't exist, create the collection from the src entityset
    */
    _mapEntitySetToProcessor: function( entitySet, processor ){
        ...
        if( processor.entityFilter ){
            ...
            // do we already have a collection for this filter?
            hash = Utils.hash( entitySet.hash() + filter.hash(), true );

            // log.debug('created filter ' + filter.toString() + ' with hash ' + hash );

            if( this._entityCollections[ hash ] ){
                collection = this._entityCollections[ hash ];
            } else {
                collection = EntitySet.createCollection( entitySet, filter );
                this._entityCollections[ hash ] = collection;
                // log.debug('new collection ' + collection.hash() + ' has ' + entitySet.entities.models.length + ' entities for ' + processor.name );
            }
            ...
            processor.set('entitySet', entitySet);
        }


        this._entitySetProcessors.add( record );

        // store the mapping between the entityset and the processor
        // an entityset can have multiple processors
        // entitySetProcessors = this._entitySetProcessors[ entitySet.id ] || createProcessorCollection();
    ...
    */
    updateSync: function( timeMs, options ){
        ...
        this._entitySetProcessors.each( function(processorRecord){
            var processor = processorRecord.get('processor');
            var collection = processorRecord.get('collection') || processorRecord.get('entitySet');
            var entityArray = collection.entities.models;

            // log.debug('executing processor ' + processor.name + ' ' + processor.get('priority') + ' with ' + collection.hash() + ' ' + entityArray.length + ' models');
            processor.onUpdate( entityArray, timeMs );

            // apply any changes to the entitySet that the processor may have received
            processor.applyChanges();

            // if the collection needs updating, then do so now
            if( collection.isModified ){
                // log.debug('updating modified collection ' + collection.hash() );
                collection.update();
            }
        });
...
function componentToString(component, indent){
    ...
    return indent + 'c' + component.name 
        ...
        + ' ' + component.hash()
        + ' ' + JSON.stringify(_.omit(component.toJSON(),'id') );
}
