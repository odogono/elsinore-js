...
//     }
    ...
    _.extend(Registry.prototype, Backbone.Events, {
        ...
        initialize: function(options,callback){
            ...
            var storage = options.storage || odgnEntity.storage.MemoryStorage;
         ...
         * @return {[type]}    [description]
         */
        useStorage: function( mw, options, callback ){
            if( mw.create ){
                var self = this;
                var storage = mw.create(this, options);
         ...
         * @return {[type]}          [description]
         */
        destroyEntity: function( entityId, callback ){
            var self = this;

            var entity = Entity.toEntity(entityId);

            // get list of components that belong to this entity
            var components = this.getEntityComponents( entityId );// this.getComponentDefs();

            return self.removeComponent( components, entity, function(err, components,entity ){

                // destroy the entity
                return self.storage.destroyEntity( entity, function(err,entity){
                    if( err ){ return callback(err); }
            ...
            // // destroy each component in turn
            // async.eachSeries( components, function(com,cb){

            //     self.removeComponent( com, entity, cb );
        ...
        getEntityComponent: function( entity, componentDef, callback ){
            entity = odgnEntity.Entity.toEntity(entity);
            entity.registry = this;
                ...
                function(entityData,entityDataCb){
                    return async.waterfall([
                        function createEntity(createEntityCb){
                            self.createEntity(options, createEntityCb);
                        },
        ...
        addComponent: function( component, entity, callback ){
            ...
            return this.instantiateComponents( component, null, function(err,components){
                ...
                return async.mapSeries( components, function(component,cb){
                    ...
                    if( err ){ return callback(err); }
                    log.debug('addComponent entity ' + entity.id + ' ' + entity.cid + ' ' + entity.get('component_bf') );
                    if( single )
     ...
     * @return {[type]}            [description]
     */
    Registry.create = function(options,callback){
        if( _.isFunction(options) ){
            callback = options; options = {};
        }
        options = options || {};
        ...
        // other
        return cr.memoryStorage = cr.useStorage( odgnEntity.storage.MemoryStorage, options, function(err,storage){
            return callback(err, cr);
        });
    };

if( typeof module !== 'undefined' && module.exports ){
    module.exports = { create:Registry.create };
}
