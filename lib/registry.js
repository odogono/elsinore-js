...
_.extend(Registry.prototype, Backbone.Events, {
    ...
    registerComponent: function( data, options ){
        ...
        return this.storage.registerComponent( componentDef, options )
            .then( function(componentDef){
                ...
                // log.debug('registered ' + JSON.stringify(data) );

                // add the ComponentDef to the registry - it should have an id

                // map from the componentDef id (int) to the componentDef
                self._componentDefs[ componentDef.id ] = componentDef;

                self.ComponentDef[ componentDef.get('name') ] = componentDef.id;
    ...
    createComponent: function( componentDefId, attrs, options ){
        ...
        componentDefId = !_.isArray(componentDefId) ? [componentDefId] : componentDefId;
        ...
        var components = componentDefId.map( function(id){
            ...
            // log.debug('creating component ' + JSON.stringify(id) ); print_ins( def );
            if( !def ){
                return null;
            }
            var component = def.create( null, options );
        ...
        // if( !componentDef ){
        //     return Promise.reject(new Error("component def '" + componentDefId + "' not found"));
        // }
        
        // instantiate the component using the componentDef
        // var component = componentDef.create( attrs, options );
        // component.registry = this;

        if( !doSave ){
            this.trigger('component:create', components, this );
            return Promise.resolve( component );
        }

        return this.storage.saveComponent( components, options )
            .then( function(components){
                self.trigger('component:create', components, self );
                return components;
            });
    },
    ...
    addComponent: function( componentDef, entity, options ){
        ...
        if( !componentDef ){//|| !ComponentDef.isComponentDef(componentDef) ){
            return Promise.reject(new Error('invalid component def passed') );
        }
