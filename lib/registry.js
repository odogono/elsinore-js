    ...
    createEntityFilter: function( entityFilter, options ){
        ...
        if( _.isArray( args[0] ) ){
            ...
            args = _.map( args, function(filterDef){
                ...
                componentIds = _.flatten(filterDef.slice(1));
                componentIds = self.getIId.apply( self, componentIds );
                return [filterType, componentIds];
            });
            // if( args.length === 1 ){ args = args[0]; }
            // log.debug('ok go ' + JSON.stringify(args) );
    ...
    */
    _mapEntitySetToProcessor: function( entitySet, processor, options ){
        ...
        if( processor.entityFilter ){
            ...
            if( debug ){ log.debug('hashed es filter ' + hash + ' ' + EntityFilter.hash( filter ) + ' ' + JSON.stringify(filter.toArray(true)) ); }
            ...
            if( this._entityViews[ hash ] ){
                ...
                if( debug ) {log.debug('new view ' + view.cid + '/' + view.hash() 
                    + ' with filter ' + EntityFilter.hash( filter ) 
    ...
    triggerEntityEvent: function( name, entity ){
        ...
        for( i=0,l=this._entitySets.length; i < l; i++ ){
            ...
            if( !entitySet.entityFilter || entitySet.entityFilter.accept( entity ) ){
                entitySet.triggerEntityEvent.apply( entitySet, arguments );// [entity,name].concat( args ) );
            }
        }
    },
});
