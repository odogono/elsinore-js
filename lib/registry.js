...
_.extend(Registry.prototype, Backbone.Events, {
    ...
    createEntity: function( components, options){
        ...
        // if( options === undefined && !_.isArray(components) && _.isObject(components) ){
            // options = components;
            // components = null;
        // }
        ...
        // log.debug('toEntity! ' + JSON.stringify(entity) );
    ...
    */
    ...
    registerComponent: function( data, options ){
        ...
        return this.storage.registerComponentDef( componentDef, options )
    ...
    */
    ...
    getComponentDef: function( defId ) {
        // log.debug('getComponentDef: ' + JSON.stringify(defId) );
    ...
    createComponent: function( componentDefId, attrs, options ){
        ...
        // assume we are saving the components if no option has been passed
        var doSave = _.isUndefined(options.save) ? true : options.save;

        var isSingle = !_.isArray(componentDefId);
        componentDefId = isSingle ? [componentDefId] : componentDefId;

        var components = this._instantiateComponents( componentDefId, attrs, options );

        if( !doSave ){
            return Promise.resolve( isSingle ? components[0] : components );
        }

        return this.storage.saveComponents( components, options )
            .then( function(components){
                var result = components.length <= 1 ? components[0] : components;
    ...
    *
    */
    _instantiateComponents: function( componentDefId, attrs, options ){
        var self = this, 
            componentDef;
        options = options || {};
        ...
        // first job - resolve the componentDefId into ComponentDef instances
        ...
        if( isSingle ){
            // we may have been passed an object which contains both attributes for the component
            // and the id of the component.
            if( _.isObject(componentDefId) && componentDefId.schema ){
                attrs = _.omit( componentDefId, 'schema' );
                componentDefId = componentDefId.schema;
            }
            componentDef = this.getComponentDef( componentDefId );
            if( !componentDef ){
                log.debug('nothing for ' + JSON.stringify(componentDefId) );
            }
            ...
            // if we are dealing with an array of component def ids, then we can safely ignore
            // the attrs argument and just return as is
            componentDef = componentDefId.map( function(id){
                // we may have been passed an object which contains both attributes for the component
                // and the id of the component.
                if( _.isObject(id) && id.schema ){
                    attrs = _.omit( id, 'schema' );
