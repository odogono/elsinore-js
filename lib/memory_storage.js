...
_.extend(MemoryStorage.prototype, Backbone.Events, {
    ...
    */
     ...
     * @return {[type]}          [description]
     */
    hasEntity: function( entityId, debug ){
        var entity = this.toEntity( entityId );
        if( debug ){
            log.debug('do i haz it ' + entityId );
            print_ins( entity );
            print_ins( this._entities );
        }
    ...
    registerComponent: function( componentDef, options ){
        ...
        return new Promise(function(resolve,reject) {
            ...
            self._componentDefsBySchemaId[ componentDef.get('schema').id ] = componentDef;
    ...
    addComponent: function(components, entity, callback ){
        ...
        return new Promise(function(resolve,reject){
            ...
            for( var i in components ){
                ...
                componentDef = self.registry.getComponentDef( component.defId );

                // store the component instance by type
                componentsByEntity = self._componentsByType[ component.defId ];
                if( !componentsByEntity ){
                    self._componentsByType[ component.defId ] = componentsByEntity = [];
                }
                ...
                entity[ componentDef.get('name') ] = component;
