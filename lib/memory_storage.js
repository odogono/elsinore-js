...
_.extend(MemoryStorage.prototype, Backbone.Events, {
    ...
    */
    ...
    destroyEntity: function(entity, options ){
        ...
        return new Promise(function(resolve,reject){
            ...
            if( !self._entitySet.hasEntity(entity) ){
                return reject(new Error('entity ' + entityId + ' not found'));
            }

            self._entitySet.removeEntity( entity );

            // add this entity to the available
            self._entitiesAvailable.push( entityId );
    ...
    */
    ...
    registerComponentDef: function( componentDef, options ){
        ...
        return new Promise(function(resolve,reject) {
            ...
            self.trigger('cdef:register', componentDef);
    ...
    */
    saveComponents: function( components, options ){
        ...
        return new Promise( function(resolve,reject){
            ...
            // var defIds = components.map(function(c){ return Component.getComponentDefId(c); });

            // log.debug('S e ' + entityId + ' ms.saveC ' + self._entitySet.cid + ' ' + defIds + ' ' + ids );
            // var clone = component.clone();
            self._entitySet.addComponent( components );
            // log.debug('E e ' + entityId + ' ms.saveC ' + self._entitySet.cid + ' ' +defIds + ' ' + ids );
            // if( entityId == 1001 )process.exit();
    ...
    */
    ...
    createEntitySet: function( options ){
        ...
        // attach to ourselves by default, so that all entity/component updates
        // are instantly reflected
        var attach = _.isUndefined(options.attach) ? true : options.attach;

        // if the es is not attached, then it can reset by default
        var reset = options.reset;
        
        return new Promise( function(resolve, reject){
            // log.debug('argle ' + JSON.stringify(options) );
            var result = EntitySet.create( options );
            result.set({id:self._entitySetId++});
            result.storage = self;
            result.registry = self.registry;

            // attach events from our own entitySet to the new entitySet
            if( attach ){
                result.attachTo( self._entitySet );
            } else if( reset ){
                self._entitySet.triggerResetTo( this );
            }
