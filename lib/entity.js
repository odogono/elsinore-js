...
var Entity = Model.extend({
    ...
    addComponent: function( component, options ){
        ...
        this.components[ component.schemaHash ] = component;
    },
    ...
    // /**
    // *   Adds all of this entities components to the otherEntity
    // */
    // applyComponents: function( otherEntity, options ){
    //     var i,len,defId;
    //     var vals = this.getComponentBitfield().toValues();
    //     for( i=0,len=vals.length;i<len;i++ ){
    //         defId = vals[i];
    //         otherEntity.addComponent( this.components[defId], options );
    //     }
    // },
    ...
    //     // log.debug('e ' + this.id + ' remC ' + componentDefId + ' ' + componentDef.getName() );
...
Entity.toEntity = function( entity, options ){
    ...
    if( Entity.isEntity(entity) ){
        options || (options = {});
        if( options.clone ){
            var cloned = Entity.create(entity.id, options);
            entity.applyComponents( cloned );
            return cloned;
        }
        return entity;
    }
