    ...
    toJSON: function( options ){
            ...
            if( relation instanceof exports.Entity ){
                ...
                if( !(exclusions && relation.match(exclusions)) ){
                    if( !relation.isNew() && relationsAsId ){
                        // log.debug('OH HEY ' + prop + ' ' + relation.type + ' ' + this.type );
        ...
        if( this.type ){
            ...
            for( i in entityDef.ER ){
                ...
                // if( this.type == 'mem_image' ) log.debug('entity.toJSON: ' + erName + ' ' + JSON.stringify(er) );
                ...
                if( er.oneToMany ){
                    ...
                    // if( this.type == 'mem_entity' )log.debug ('hoopy ' + erName + ' ' + JSON.stringify(exclusions) );
                    if( !relation.isNew() && relationsAsId ){
...
exports.create = function( entityType, attrs, options ) {
    ...
    if( _.isObject(entityType) && !entityType.__super__ ){
        options = attrs;
        attrs = entityType;
        entityType = attrs.entityType || attrs.type;
    }
    else if( _.isObject(entityType) && _.isObject(attrs) && (entityType.entityType || entityType.type) ){
        entityType = entityType.entityType || entityType.type;
    }
...
exports.registerEntity = function( entityDef, entity, options ){
    ...
    if( _.isObject(entity) && entity.__super__ ){
        ...
        if( _.isString(entityDef) ){
            entityDef = { entityType:entityDef, type:entityDef };
            entity.type = entityDef.entityType;
        ...
        entityDef.entityType = entityDef.type = (entityDef.type || entityType);
    }
