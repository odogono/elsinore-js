...
var Entity = exports.Entity = Backbone.Model.extend({
            ...
            if( options && options.changes ){
                for( var attrName in options.changes ){
                    if( (existing = this.get(attrName)) ){
                        if( !_.isUndefined(existing._refCount) )
                            existing._refCount++;
                    }
                    
                    if( (previous = this.previous(attrName)) && previous != existing && previous._refCount )
                        previous._refCount--;
                }
            }
        });
     ...
     */
    setEntity: function( entityType, instance, options ){
        this.set( entityType, instance, options );
    },
    ...
    set: function(key, value, options) {
        ...
        entityDef = exports.ids[this.entityType || attrs.entityType];

        // if( options && options.debug ) log('set ' + JSON.stringify(attrs));
        // console.log('entity.set ' + JSON.stringify(attrs));
    ...
    parse: function(resp, xhr, options){
        ...
        var entityDef = exports.ids[ resp.entityType || this.entityType ];
        ...
        var associateRelations = function(data, options){
            ...
            var entityType = data.entityType || options.entityType || self.entityType;

            if( !_.isUndefined(entityType) ) 
                data.entityType = entityType;

            if( !(entityDef = (options.entityDef || exports.ids[ entityType ])) )
                return;

            // if( debug ) log('calling it ' + entityType + ' ' + JSON.stringify(data) );
            if( removeId ){
                delete data.id;
            }
            ...
            var mapErIdToItems = function(item){
                item = _.isObject(item) ? item : origResp[item];
                return associateRelations( item, {entityType:er.oneToMany} );
            };

            var mapEidToItems = function(eid){
                if( !_.isObject(eid) )
            ...
            for( i in entityDef.ER ){
                ...
                if( er.oneToOne ){
                    ...
                    if( _.isArray(erId) ){
                        ...
                        items = _.map( erId, mapErIdToItems );/*function(item){
                            item = _.isObject(item) ? item : origResp[item];
                            return associateRelations( item, {entityType:er.oneToMany} );
                        });//*/
                        ...
                        if( origResp[erId] ){
                            ...
                            erId.items = _.map( erId.items, mapEidToItems );/*function(eid){
                                if( !_.isObject(eid) )
                                    return associateRelations( origResp[eid] );
                                return eid;
                            });//*/
    ...
    removeID: function(options){
        ...
        var entityDef = exports.ids[this.entityType];
        options = options || {};
    ...
    toJSON: function( options ){
        ...
        // log.debug('toJSON for ' + this.type );
        // if( this.type === 'mem_image' ) print_var( attrs );
        for (var prop in attrs) {
            ...
            if( relation instanceof exports.Entity ){
                ...
                // if( options.debug ) log.debug('checking ' + relation.id + ' for exclude');
                if( !(exclusions && relation.match(exclusions)) ){
                    if( !relation.isNew() && relationsAsId ){
                        result[prop+'_id'] = relation.id;
                        delete result[prop];
                        // print_var(result);
        ...
        if( this.entityType ){
            entityDef = exports.ids[this.entityType];
            ...
            for( i in entityDef.ER ){
                ...
                if( er.oneToMany ){
                    ...
                    if( !relation.isNew() && relationsAsId ){
                        relation[erName+'_id'] = relation.id;
        ...
        if( options._refCount )
            result._refCount = this._refCount;
    ...
    _flattenER: function( entityDef, outgoing, options ){
        if( !entityDef || entityDef.ER )
            return;
        ...
        for( i in entityDef.ER ){
            var er = entityDef.ER[i];
            var erName = (er.name || er.oneToMany || er.oneToOne ).toLowerCase();
            
            if( er.oneToOne ){
                if( doRecurse && (child = this.get(erName)) && child._refCount <= maxRefCount ){
    ...
    flatten: function( options ){
        ...
        if( !result[id] ){
            if( options.toJSON ){
                ...
                outgoing.entityType = this.entityType;
                if( this.id )
                    outgoing.id = this.id;
                else
            ...
            this._flattenER( entityDef, outgoing, options );
    ...
    match: function( cases, options ){
        ...
        for( i in cases ){
            ...
            if( _.isObject(kase) ){
                for( p in kase ){
                    if( p === 'entityType' && this.entityType == kase[p] )
                        return true;
...
exports.isEntity = function( obj ){
    return _.isObject(obj) && obj.__super__;
};


// returns constituent parts of an id - the entityType and id
exports.splitId = function( id ){
    var result = { id:id };
    if( id.indexOf('.') != -1 ){
        result.entityType = id.substring( 0, id.indexOf('.') );
    }
    return result;
};
...
*/
exports.create = function( entityType, attrs, options ) {
    var i,er,result;
    var collectionName, props;
    var debug = options && options.debug;

    // log.debug('creating with ' + JSON.stringify(arguments) );

    // TODO - this is a super-hateful mess, refactor

    if( _.isObject(entityType) && !entityType.__super__ ){
        options = attrs;
        attrs = entityType;
        entityType = attrs.entityType;
    }
    else if( _.isObject(entityType) && _.isObject(attrs) && entityType.entityType ){
        ...
        attrs = entityType;
        entityType = attrs.entityType;
        if( !entityType && attrs.id && attrs.id.indexOf('.') > 0 )
            entityType = attrs.id.substring( 0, attrs.id.indexOf('.') );
    }
    else if( attrs ) {
        if( !_.isObject(attrs) )
            attrs = { id:attrs };
    ...
    if( _.isObject(entityType) ){
        if( entityType.entityType )
            entityType = entityType.entityType;
    }
    else {
        if( _.isString(entityType) && entityType.indexOf('.') != -1 ){
            attrs.id = entityType;
    ...
    if( entityType === undefined ){
        throw new Error('undefined entityType' );
    }

    // look for ER properties
    var entityDef = exports.ids[entityType];
    var erProperties = {};
    ...
    for( i in entityDef.ER ){
        ...
        if( er.oneToMany ){
            ...
            if( attrs[collectionName] ){
                erProperties[collectionName] = {attrs:attrs[collectionName], entityType:er.oneToMany, inverse:(er.inverse||entityType)};
                delete attrs[collectionName];
            }
        }
    }

    if( !entityDef._entityClass )
        log.error( 'could not find Entity in ' + entityDef.entityType );

    result = new entityDef._entityClass( attrs, options );

    result.entityType = entityType;
    result._refCount = 0;
    result._entityDef = entityDef;
...
// normalises the entityType of an entity
exports.getEntityFromType = function( entityType ){
    if( entityType === null || entityType === undefined )
        return null;
    if( _.isObject(entityType) && entityType.entityType )
        entityType = entityType.entityType;
    if( !_.isString(entityType) )
        return null;
    if( exports.ids[entityType.toLowerCase()] ){
        return exports.ids[entityType.toLowerCase()];
    }
    if( exports[entityType.toUpperCase()] ){
        return exports[entityType.toUpperCase()];
    }
    if( exports['TYPE_' + entityType.toUpperCase()] ){
        return  exports.ids[exports['TYPE_' + entityType.toUpperCase()]];
    }
    // look for schema
    return null;
};
...
exports.getER = function( entityType ){
    entityType = exports.getEntityFromType( entityType );
    return entityType.ER;
};
 ...
 * 
 * @private
 * @param  {[type]} entityDef [description]
 * @param  {[type]} options   [description]
 * @return {[type]}           [description]
 */
exports._registerEntityType = function( entityDef, options ){
    options = (options || {});
    entityDef.name = entityDef.name || entityDef.entityType;
    
    if( !entityDef.entityType ){
        log.debug('no entityType supplied ' + JSON.stringify(entityDef) );
        return null;
    }
    ...
    if( entityDef.Entity ){
        entityDef._entityClass = entityDef.Entity;
        delete entityDef.Entity;
    }
    else if( !entityDef._entityClass ){
        // log.debug('no _entityClass supplied with entityDef ' + entityDef.entityType );
        entityDef._entityClass = exports.Entity.extend({});
    }
    ...
    // create a standard create function if one is not supplied
    if( !entityDef.create ){
        entityDef.create = function(attrs, options){
            return exports.create( entityDef.entityType, attrs, options );
        }
    }

    if( !entityDef.parse ){
        entityDef.parse = function(resp,options){
            var ent = exports.create( entityDef.entityType, options);
            ent.set( ent.parse(resp,options) );
            return ent;
        }
    }
    ...
    exports.ids[entityDef.entityType] = entityDef;
    exports.names[entityDef.entityType] = entityDef.name;
    // log.debug('exporting ' + 'TYPE_' + _.underscored(entityDef.name).toUpperCase() );
    exports['TYPE_' + _.underscored(entityDef.name).toUpperCase()] = entityDef.entityType;

    return entityDef;
}

exports._unregisterEntityType = function(entityDef){
    var entityType = _.isObject(entityDef) ? entityDef.entityType : entityDef;
    entityDef = exports.ids[ entityType ];

    if( !entityDef )
        return;

    delete exports['TYPE_' + _.underscored(entityDef.name).toUpperCase()];
    delete exports.names[entityDef.entityType];
    delete exports.entities[entityDef.name];
    delete exports.ids[entityDef.entityType];    
    delete exports[entityDef.name];
};


exports.unregisterEntity = function(entityDef){
    exports._unregisterEntityType( entityDef );
};

exports.unregisterAll = function(){
    for( var id in exports.ids ){
        exports.unregisterEntity( id );
    }
}
...
exports.registerEntity = function( entityDef, entity, options ){
    ...
    if( _.isObject(entity) && entity.__super__ ){
        ...
        if( _.isString(entityDef) ){
            entityDef = { entityType:entityDef, entityType:entityDef };
            entity.entityType = entityDef.entityType;
            // entity.entityType = entityDef.type;
        }
        entityDef._entityClass = entity;
        options = options || {};
    }
    ...
    if( exports.ids[entityDef.entityType] ){
        return exports.ids[entityDef.entityType];
    }

    if( !entityDef ){
        log.debug('no entityDef came back');
    }

    entityDef = exports._registerEntityType( entityDef, options );
    
    if( entity ){
        entity._entityDef = entityDef;
        if( !entity.create ){
            entity.create = entityDef.create || options.create;
        }
    }
    return entityDef;
};
