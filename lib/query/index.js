...
Query.resolveEntitySet = function resolveEntitySet( context, entitySet, compileOnly ){
    ...
    if( _.isArray(entitySet) ){
        ...
        // if( op === Query.VALUE ){
        //     if( entitySet[1] === Query.ROOT ){
        //         return context.entitySet;
        //     }
        // }
        // else if( op === Query.AND || op === Query.ALL || op === Query.NONE ){
            
        //     // turn the expression into an array of entity filter objects
        //     result = gatherComponentFilters( context, entitySet );
        //     // trying to compile component filters didn't work out
        //     if( !result ){
        //         return null;
        //     }
        //     entitySet = [ Query.ENTITY_FILTER, result ];
        // }
...
function commandPluck( context, componentIds, attributes, options ){
    ...
    attributes = Query.valueOf( context, attributes, true );
...
function commandSelectById( context, entityIds ) {
    ...
    entityIds = Query.valueOf( context, entityIds );
...
function commandAliasGet( context, name ){
    var value;
    if( context.debug ){ printIns( context.alias ); }
    context.alias = (context.alias || {});
    name = Query.valueOf( context, name, true );
    value = context.alias[ name ];
    if( !value ){
...
function commandFilter( context, entityFilter, filterFunction, options ){
    ...
    if( Entity.isEntity(entitySet) ){
        entity = entitySet; entitySet = null;
    }
...
Query.commandFunction = function commandFunction( op ){
    ...
    switch( op ){
        ...
        case Query.ALIAS:
            result = commandAlias;
            break;
        case Query.ALIAS_GET:
            result = commandAliasGet;
            break;
        case Query.WITHOUT:
            result = commandWithout;
            break;
        case Query.SELECT_BY_ID:
            result = commandSelectById;
            break;
        default:
...
function executeCommand( context, op, args ){
    ...
    if( !args ){
        ...
        // log.debug('split op into ' + Utils.stringify(op) );
        // var altArgs = op.slice(1);
        args = _.rest( op );
        op = op[0];
        // log.debug('args = ' + Utils.stringify(args) );
        // log.debug('args = ' + Utils.stringify(altArgs) );
    }
    ...
    commands = _.reduce( commands, function(result,command){
        ...
        switch( op ){
            ...
            case Query.PLUCK:
                ...
                if( command[1] ){
                    command[1] = resolveComponentIIds( context, command[1] ); }
                result.push( command );
                break;
            default:
                result.push( command );
...
Query.execute = function executeQuery( entity, commands, options ){
    ...
    if( context.debug ){log.debug('commands:'); printIns( commands,1 ); }
