...
function EntityFilterTransform( type, registry, entity, entityBitField, filterBitField ){
    ...
    // log.debug('EFT ' + entityBitField.toJSON() + ' ' + filterBitField.toJSON() );
    if( isInclude ){
...
Query._filterEntitySet = function( context, entitySet, entityFilter, options ){
    var entities,result;
...
Query.commandEntityFilter = function( context, entityFilter, options ){
    var entitySet, entity, entities, registry, filtersLength,result,ii,ebf,filter;
    ...
    if( entity ){
        ...
        // if( true || context.debug ){ log.debug('commandEntityFilter> entity ' + context.entity.getEntityId() ); }
        // filter the entity using the component filters - returns a boolean
        ebf = entity.getComponentBitfield();
        for( ii=0;ii<filtersLength;ii++ ){
            filter = entityFilter.filters[ii];
            // printIns( filter );
            if( filter.type == EntityFilter.INCLUDE ){
                entity = EntityFilterTransform( filter.type, registry, entity, ebf, filter.bitField );
            }
            else if( !EntityFilter.accept( filter.type, ebf, filter.bitField, false ) ){
                return (context.last = [ Query.VALUE, null ]);
            }
        }
...
Query.commandFunction = function commandFunction( op ){
    ...
    switch( op ){
        ...
        case Query.ENTITY_FILTER:
            result = Query.commandEntityFilter;
            break;
...
Query.compile = function compileQuery( context, commands, options ){
    ...
    for( ii=0,len=commands.length;ii<len;ii++ ){
        ...
        if( entityFilters.length > 0 ){
            result.commands.push( [ Query.ENTITY_FILTER, EntityFilter.combine(entityFilters) ] );
            entityFilters = [];
        }
        if( ii < len ){
...
Query.buildContext = function( entity, query, options ){
    ...
    context.registry = rootObject ? rootObject.getRegistry() : null;
