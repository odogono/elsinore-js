...
function gatherComponentFilters( context, expression ){
    ...
    switch( expression[0] ){
        ...
        case Query.AND:
            ...
            //     sub = gatherComponentFilters(context,sub);
            //     if( !sub )
...
function commandSelectById( context, entitySet, entityIds ) {
...
function compileCommand( context, command ){
    ...
    switch( op ){
        ...
        case Query.NONE:
            ...
            break;
        case Query.AND:
            command = (Query.resolveEntitySet( context, command, true ) || command);
            break;
        default:
            // log.debug('missed compile op ' + op + ' ' + JSON.stringify(_.rest(command)));
            break;
    }
