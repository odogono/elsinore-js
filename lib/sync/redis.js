...
var RedisStorage = function(config, options) {
...
_.extend( RedisStorage.prototype, {
    ...
    saveEntity: function( redisHandle, entity, options, callback ){
        ...
        // options.debug = entity.type == 'build_job';
        if( options.debug ) log.debug('saving entity ' + entity.type );
        ...
        if( !entity.id ){
            ...
            exports.generateUuid(function(err,id){
                ...
                log.debug('saving changed entity ' + entity.id);
        ...
        serialised._et = entity.type;

        delete serialised.id;
        if( options.debug ) log.debug( 'saving serialised: ' + JSON.stringify( serialised ) );

        key = keyPrefix + ':' + entity.id;
        if( options.debug ) log.debug( 'prefix key is ' + key );        
        ...
        if( keys = _.result( entity, 'storeKeys' ) ){
            if( options.debug ) log.debug('storeKeys: ' + JSON.stringify(keys) );
        ...
        var setIndexes;
        if( (setIndexes = (entity.getSetIndexes && entity.getSetIndexes())) ){
            for( var indexKey in setIndexes ){
                setIndexes[indexKey].forEach(function(val){
                    if( entity.get(indexKey) == val )
                        multi.sadd( keyPrefix + /*':' + entity.type +*/ ':' + indexKey + ':' + entity.get(indexKey), entity.id );
                    else
                        multi.srem( keyPrefix + /*':' + entity.type +*/ ':' + indexKey + ':' + val, entity.id );    
                });
            }
        }

        // add the entity id to its entity type collection
        multi.sadd( keyPrefix + ':et:' + entity.type, entity.id ); // entity_type        

        if( (collection = entity.entityCollection) && (storeId = collection.getStoreId()) ){
            if( options.debug ) log.debug('adding to entityCollection set ' + entity.id);
...
_.extend( RedisStorage.prototype, {
    ...
    retrieveContainerEntities: function( collection, resultSet, options, callback ){
        ...
        if( debug ) log.debug('retrieving entities by ' + JSON.stringify(collection.attributes) );
...
_.extend( RedisStorage.prototype, {
    ...
    update: function(model, options, callback) {
        ...
        Step(
            ...
            function(err,replies){
                if( options.debug ) log.debug('entity id is now ' + model.id + '(' + model.cid + ')');
                callback(err, model);
            }
        );//*/
        /*
    ...
    retrieveEntityByQuery: function( query, resultSet, options, callback ){
        ...
        var ldlKey = null; //options.key_prefix + ":status:" + jstonkers.Status.LOGICALLY_DELETED;
        ...
        if( options.debug ) log.debug( key );
    ...
    retrieveEntityById: function( entity, resultSet, options, callback ){
        ...
        multi.exec( function(err, replies){
            ...
            type = entityAttr._et || (_.isObject(entity) ? entity.type : null);
            ...
            if( !entityDef && options.entityDefHint ){
                entityDef = Entity.ids[options.entityDefHint.type];
            }
            ...
            if( entityDef.oneToMany ){
                ...
                // log.debug('looking for members of ' + entityKey + ':items' );
                // log.debug( JSON.stringify(entityDef) );
                var fetchSetEntry = {key:entityKey + ':items', name:'items', _et:type, callback:function(params,result){
                    entityAttr.items = result;
                }};
                if( options.query && options.query.items ){ // NOTE: really lame
                    fetchSetEntry.query = options.query.items;
                }
                // fetchSetEntry
                resultSet.fetchSetList.push( fetchSetEntry );
            }
                    ...
                    if( er.oneToMany ){
                        ...
                        if( targetId ){
                            if( (options.fetchRelated || resultSet.depth < options.depth) && !resultSet.results[targetId] ){
                                ...
                                if( debug ) log.debug('2 adding ' + name + ' ' + targetId + ' ' + entityDef.type );
                        ...
                        if( targetId && (options.fetchRelated || resultSet.depth < options.depth) && !resultSet.results[targetId] ){
                            ...
                            if( options.debug ) log.debug('3 add ' + targetId + ' to fetchEntityList' );
    ...
    retrieveEntities: function( resultSet, options, callback ){
        ...
        options.debug = true;
        // log.debug('retrieving entities with options: ' + JSON.stringify(options) );

        if( resultSet.fetchSetList.length > 0 ){
            
            item = resultSet.fetchSetList.shift();

            log.debug('a. fetchSetList ' + JSON.stringify(item) );
            if( _.isObject(item) ){
                var multi = self.client.multi();
                var setKey = item.key;

                if( item.query ){
                    setKey = options.key_prefix + ':query';
                    // build a set key for the query
                    // "isbvb:bs:status:0"
                    multi.sinterstore( setKey, item.key, "isbvb:bs:status:0" );
                    // self.client.sinter( item.key )
                }

                // get the members of the resultant set
                multi.smembers( setKey );

                multi.exec(function(err, replies){
                    if( err ) throw err;
                    // the last entry will be the results
                    var result = _.last(replies);

                    if( err ) throw err;
                    // if( options.debug ) 
                    log.debug('a. result of set ' + item.key + ' ' + JSON.stringify(result) );
                    ...
                    for( i in result ){
                        ...
                        if( result[i] && !resultSet.results[result[i]] ){
                            if( options.debug ) log.debug('1 add ' + result[i] + ' to fetchEntityList' );
            ...
            if( options.debug ) log.debug('b. retrieveEntities : entity ' + entityId );
            ...
            // }
            // else {
                if( options.debug ) log.debug('b. going for retrieve of ' + entityId );
                self.retrieveEntityById( entityId, resultSet, retrieveOptions, function(err,data){
                    ...
                    log.debug('b. retrieved ' + JSON.stringify(data) );
                    // print_var( resultSet );
                    // store result
                    resultSet.results[ entityId ] = data;
                    // re-check if we need to fetch anything else
                    log.debug('b. onwards ' + JSON.stringify(resultSet.fetchSetList) )
                    self.retrieveEntities( resultSet, options, callback );
                });
            // }
            
        } else {
            if( options.debug ) log.debug('finished fetchEntityList retrieve');
    ...
    find: function(model, resultSet, options, callback) {
        ...
        if( model.id ){
            log.debug('starting with ' + model.id );//+ ' ' + JSON.stringify(model.get('query')) );
            // console.log( model );
            resultSet.fetchEntityList.push( model.id );
        }

        /*if( options.query ){
            if( options.debug ) log.debug('going for retrieve with query ' + JSON.stringify(options.query) );
...
// var fwdCount = 0;
// 
exports.initialize = function(config, options){
    store = new RedisStorage( config );
    return store;
}

exports.sync = function(method, model, options) {
    var config = _.extend( _.clone(store.config), options );
    var concluded = false;
    var resp, modelID;
...
exports.createSioTokenForSession = function( sessionId, callback ){
    ...
    // exports.generateUuid( {uuid_key:'uuid_sio'}, function(err,token){
        store.client.set( prefix + ':siot:' + token, sessionId, function(err, result){
            if( err ) throw err;
            log.debug('set siotoken to ' + token );
            callback(err, token );
        });
    // })
}
...
exports.getSessionIdFromSioToken = function( sioToken, callback ){
    ...
    store.client.get( prefix + ':siot:' + sioToken, function(err, sessionId){
        if( err ) throw err;
        log.debug('retrieved siotoken ' + sioToken );
