...
_.extend( RedisStorage.prototype, {
    ...
    saveEntity: function( redisHandle, entity, options, callback ){
        ...
        // entity status index
        // TODO : things like status being keys should be defined in the entity
        ...
        if( resultSet.fetchSetList.length > 0 ){
            ...
            if( _.isObject(item) ){
                self.client.sdiff( item.key, ldlKey, function(err,result){
                    if( err ) throw err;
                    // if( options.debug ) 
                    // log.debug('result of set ' + item.key + ' ' + JSON.stringify(result) );
            ...
            if( options.debug ) log.debug('retrieveEntities : entity ' + entityId );
            ...
            // }
            // else {
                if( options.debug ) log.debug('going for retrieve of ' + entityId );
