...
exports.Command = Entity.Entity.extend({
    ...
    /**
     * Proxy command for adding commands to the queue
     * @param {[type]} cmd     [description]
     * @param {[type]} options [description]
     */
    addCommand: function( cmd, options ){
        if( this.entityCollection )
            this.entityCollection.add( cmd );
    },
...
exports.CommandQueue = Entity.EntityCollection.extend({
    ...
    process: function(options,callback){
        ...
        options = (options || {});
        var isSerial = this.get('isSerial');
        var cmdOptions = {};
            ...
            function buildExecuteList(done){
                ...
                if( isSerial ){
                    if( !self.currentCommand ){
                        self.currentCommand = _.first( executeList );
                        self.currentCommand.isExecuting = true;
                        self.currentCommand.isFinished = false;
                        self.currentCommand.commandQueue = self;

                        log.debug('executing cmd ' + self.currentCommand.entityType + '(' + self.currentCommand.cid + ')' );
                        self.currentCommand.execute( cmdOptions, function(err){
                            if( err ){
                                log.error('error executing cmd ' + self.currentCommand.entityType + '(' + self.currentCommand.cid + ') : ' + err );
                            } else 
                                log.debug('finished executing cmd ' + self.currentCommand.entityType + '(' + self.currentCommand.cid + ')' );
                            self.currentCommand.isExecuting = false;
                            self.currentCommand.isFinished = true;
    ...
    startRunLoop: function(options, callback){
        ...
        {
            log.info( (this.get('isSerial') ? 'serial' : 'non-serial') + ' command queue loop running at ' + fps + 'fps');
...
exports.create = function( attrs, options ){
    ...
    result.entityType = exports.entityType;
    return result;
}


Entity.registerEntity( exports.entityType, exports.CommandQueue, {oneToMany:true,create:exports.create} );