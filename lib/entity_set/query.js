...
_.extend( Query, {
    ...
    WITHOUT: 25,
    NOOP: 26,
    AND_FILTER: 27, // combines several filters into one - DEPRECATED
    LEFT_PAREN: 28,
    RIGHT_PAREN: 29
});
...
function resolveEntitySet( context, entitySet ){
    ...
    if( _.isArray(entitySet) ){
        ...
        // if( entitySet[0] === Query.AND ){
            // entitySet[0] = Query.AND_FILTER;
            // this is a combination of entitySet attributes
            // context = _.extend( {}, context, {resolveEntitySet:true} );
        // }
...
function commandComponentFilterAnd( context ){
    var filters, entitySet, entities, result;
    var EntityFilter = require('../entity_filter');

    filters = _.rest( arguments );

    // TODO: consider case where only an entity is passed
    entitySet = context.entitySet;

    // convert filter arguments
    filters = _.map( filters, function( filter ){
        if( filter[1] !== Query.ROOT ){
            throw new Error('invalid filter entitySet arg ' + Utils.stringify(filter) );
        }
        filter[2] = componentsToBitfield( context, filter[2] );
        return filter;
    });

    // TODO : combine identical type filters into one

    entities = _.reduce( entitySet.models, function(result, entity){
        var i,len,fbf,ebf,type;
        ...
        for( i=0,len=filters.length;i<len;i++ ){
            ...
            if( !EntityFilter.accept( type, ebf, fbf, false) ){
                // rejected - end
                return result;
            }
        }
...
function valueOf( context, value, shouldReturnValue ){
    var command;
    if( context.debug ){ log.debug('incoming value ' + Utils.stringify(value) ); }
    if( _.isArray(value) ){
        ...
        command = value.shift();
        if( context.debug ){ log.debug('valueOf for cmd ' + command + ' ' + Utils.stringify(value) )}
        value = executeCommand( context, command, value );
        if( value[0] === Query.VALUE ){
            return value[1];
        }
    }
...
function executeCommand( context, op, args ){
    ...
    switch( op ){
        ...
        case Query.PLUCK:
            ...
            break;
        case Query.AND_FILTER:
            result = commandComponentFilterAnd.apply( null, args );
            break;
        case Query.ALL:
