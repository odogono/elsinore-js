...
exports.initialise = function(){
    schemaEnv = exports.env = JSV.createEnvironment();
    schemaEnv.setOption("strict", true);
}
...
exports.register = function( schemaId, path ){
    ...
    if( _.isObject(schemaId) ){
        local = localSchema = exports.env.createSchema( schemaId );
    } else {
        var data = _.isObject(path) ? path : require( path );
        local = exports.env.createSchema( data, local, schemaId );
    }
    return local;
}
...
exports.propertyNames = function(schemaUri){
    var properties = exports.properties(schemaUri);
    return properties ? _.keys(properties) : null;
}

exports.properties = function(schemaUri){
    // var schema = _.isObject(schemaUri) && schemaUri.type ? 
    //             Entity.ids[this.type]._schema : 
    var schema = _.isObject(schemaUri) ? schemaUri : schemaEnv.findSchema(schemaUri);
    
    if( schema )
        return schema._attributes.properties;
    return null;
}
...
exports.getSchemaValue = function(schemaUri, referringUri, options){
    ...
    // log( schemaUri + ' ' + schemaUriComponents.reference );

    // if( options ) log('finding ' + schemaUri + ' ' + referringUri);
