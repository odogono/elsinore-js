...
exports.EntityCollection = entity.Entity.extend({
    ...
    initialize: function(){
        ...
        if( !this.items ){
            ...
            .on('remove', function(model){
                ...
                // log.debug('reset over here');
                // models added during reset don't have the usual add event triggered
                var model;
                for( var i in this.models ){
                    model = this.models[i];
                    // log.debug('resetting models ' + model.cid + ' ' + model.collection );

                    // TODO: why was this following condition being tested?
    ...
    createItemsModel: function( attrs, options ){
        ...
        if( collection && !attrs.entityType ){
            attrs.entityType = collection.getEntityType();
        }
        // console.log('creating items model ' + attrs.entityType );
        if( attrs.entityType ){
            var entityDef = entity.getEntityFromType(attrs.entityType)
            var create = entityDef.create || entity.create;
            var result = create( attrs, options );
            return result;
        }
    ...
    getEntityType: function(){
        return this.entityType || this.get('entityType');
    },
    ...
    set: function(key, value, options) {
        ...
        if( _.isArray(attrs) ){
            ...
            // console.log('resetting with ' + JSON.stringify(attrs) );
        ...
        // log.debug('entC setting ' + JSON.stringify(key) );
        if( attrs.items ){
            attrs.items = _.compact(attrs.items);
            // log.debug('set entC ' + JSON.stringify(attrs) );
    ...
    parse: function(resp, xhr){
        ...
        if( resp.entity ){
            ...
            this.entityType = entityDef.entityType;
    ...
    reset: function( models, options ){
        ...
        this.items.each( function(i){
            if( i.entityType === undefined )
                i.entityType = self.getEntityType();
            // console.log('setting ' + JSON.stringify(i) );
            // if( options.inverse ){
    ...
    toJSON: function( options ){
        ...
        if( includeCounts ){
            ...
            // result.page_count = Math.ceil(result.item_count / this.get('page_size') );
            // log.debug('adding counts ' + result.page_size + ' ' + this.get('page_size') );
        } else {
