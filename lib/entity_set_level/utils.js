...
function readStream( db, options ){
    ...
    return new Promise( function(resolve){
        ...
        // log.debug('created db.readStream ' + cid );
        stream
            .on('data', function(data){
            ...
            .on('close', function(){
                ...
                // if( debug ){ 
                    ...
                    // else {
                        // log.debug('closed? ' + stream.cid + ' ' + cid ); 
                    // }
                    // }
                // if( options.pauseable && stream._waiting )
                // if( !stream._isPaused ){
                    // if( stream.cid == 'rs388' ){
                    //     log.debug('!!! closed ' + stream.cid + ' ' + cid + ' ' + stream._isPaused ); 
                    //     // printIns( stream );
                    // }

                // if we are letting this stream pause, then allow
                // it to resolve the result
                // if( !stream._pauseable ){
                    // log.debug('!!! closed ' + stream.cid + ' ' + cid + ' ' + stream._isPaused ); 
                    return resolve( result );
                // }
                // }
            ...
            .on('end', function(){
                // if( stream.cid == 'rs388' ){
                    // log.debug('!!! end ' + stream.cid + ' ' + cid + ' ' + stream._isPaused ); 
                    // printIns( stream );
                // } 
                // else {
                    // log.debug('end? ' + stream.cid + ' ' + cid ); 
                // }
            })
    });
}
