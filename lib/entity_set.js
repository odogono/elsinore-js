...
var EntitySet = Backbone.Model.extend({
    ...
    */
    map: function( entityFilter, dstEntitySet, options ){
        var e,i,elen,len;
        var entity;
        options || (options = {});
        ...
        dstEntitySet.setEntityFilter( entityFilter );
        ...
        // for( e=0,elen=srcEntitySet.entities.models.length;e<elen;e++ ){
        //     entity = srcEntitySet.entities.models[e];

        //     for( i=0,len=entityFilter.length;i<len;i++ ){
        //         entity = entityFilter[i].transform( entity );
        //         // the filter may return null if the entity is completely unacceptable
        //         if( entity ){
        //             dstEntitySet.addEntity( entity, opOptions );
        //         }
        //     }
        // }
    ...
    setEntityFilter: function( entityFilter, options ){
        ...
        entityFilter = this.getRegistry().createEntityFilter.apply( this.getRegistry(), arguments );
...
var EntityCollection = Backbone.Collection.extend({
    setEntitySet: function(entitySet){
        ...
        this.listenTo( entitySet, 'entity:add', this.onEntityAdd );
        this.listenTo( entitySet, 'entity:remove', this.onEntityRemove );
    ...
    onEntityAdd: function(entity){
        ...
        if( this.entityFilter && this.entityFilter.length > 0 ){
            for( i=0,len=this.entityFilter.length;i<len;i++ ){
                accept &= this.entityFilter[i].accept( entity );
            }
        }
        if( accept ){
            this.add( entity );
...
EntitySet.createCollection = function( entitySet, entityFilter, options ){
    ...
    if( entityFilter && !_.isArray(entityFilter) ){
        entityFilter = [ entityFilter ];
    }
    ...
    hash = _.result( entitySet, 'hash' );

    if( entityFilter && entityFilter.length > 0 ){
        for( i=0,len=entityFilter.length;i<len;i++ ){
            hash += entityFilter[i].toString();
        }
    }

    hash = Utils.hash( hash, true );
