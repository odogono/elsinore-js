...
var EntitySet = Backbone.Model.extend({
    ...
    */
    ...
    at: function(index){
        return new Promise( function(resolve, reject){
            return resolve( this.entities[index] );
        })
    },
            ...
            next: function(){
                return new Promise( function(resolve, reject){
                    if( nextIndex < self.entities.length ){
                        return resolve( self.entities[nextIndex++] );
                    }
        ...
        return {
            next: function(){
                return nextIndex < self.entities.length ?
                    { value: self.entities[nextIndex++], done:false }:
                    { done:true };
            }
        }
    },
    ...
    */
    ...
    _createEntity: function( returnId ){
        var result;
        var entityId = this.getRegistry().createId();//self._entitiesAvailable.length > 0 ?
...
function addCommand( cmdBuffer, type, arg/*entityId|component*/, options ){
    ...
    switch( type ){
        ...
        case CMD_COMPONENT_UPDATE:
            ...
            // log.debug('addCommand.COMPONENT ' + entityId );
            break;
        default:
            // NO-OP
            return;
    ...
    // log.debug('so adding ' + JSON.stringify(entityBuffer) + ' to ' + entityId );
