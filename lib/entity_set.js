...
var EntitySet = Backbone.Collection.extend({
    ...
    hash: function(){
        return EntitySet.hash( this, this.query );
    },
    ...
    */
    ...
    reset: function( entities, options ){
        ...
        for( i=0,l=entities.length;i<l;i++ ){
            entity = entities[i];
            if( EntitySet.isEntityOfInterest(this, entity, this.query) ){
                this.add( entity );
            }
        }
    ...
    */
    triggerEntityEvent: function( name, entity ){
        ...
        if( this.query && !this.query.accept( entity ) ){
            return false;
        }
        // log.debug('accepting EE on ' + this.cid+'/'+this.id );
        if( this.views ){
            _.each( this.views, function(view){
                if( !view.query || view.query.accept( entity ) ){
...
EntitySet.map = function( srcEntitySet, query, dstEntitySet, options ){
    ...
    if( query ){
        EntitySet.setEntityFilter( dstEntitySet, query );
    }
