...
var EntitySet = Backbone.Collection.extend({
    ...
    */
    pluck: function( entityFilter, attr ){
        ...
        return _.map( view.models, function(entity){
            var component = entity.components[componentIId];
            return component.get.call( component, attr );
        });
    },

    query: function( entityFilter ){
        var query = Query.create( this );
        if( entityFilter ){
            return query.select( entityFilter );
        }
        return query;
    },

    _runQuery: function( entityFilter, filters, attrFilter ){
        var registry = this.getRegistry();
        // console.log('_runQuery with ' + this.type + ' ' + entityFilter.type );
        var result = registry.createEntitySet(null, {register:false});

        // if( attrFilter ){
        //     if( !entityFilter ){
        //         entityFilter = EntityFilter.create();
        //     }
        //     entityFilter.add( EntityFilter.USER, function accept(entity,options){
        //         var result = true;
        //         _.each( attrFilter, function( attrs, componentIId ){
        //             var com = entity.components[ componentIId];
        //             if( !com ){ return; }
        //             console.log('looking at componentIId ' + componentIId + ' ' + JSON.stringify(attrs) );

        //             _.each( attrs, function( value, attr ){
        //                 if( com.get(attr) !== value ){
        //                     console.log( attr + ' ' + com.get(attr) +  ' != ' + value + '')
        //                     result = false;
        //                     return;
        //                 }
        //             });
