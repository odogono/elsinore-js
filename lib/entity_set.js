...
EntitySet.setEntityFilter = function( entitySet, entityFilter, options ){
    ...
    if( !_.isArray(entityFilter) ){
        entitySet.entityFilters = [ entityFilter ];
    } else {
        entitySet.entityFilters = entityFilter;
    }

    // check that entities are still allowed to belong to this set
    EntitySet.evaluateEntities( entitySet, entityFilters );
...
EntitySet.isEntityOfInterest = function( entitySet, entity, additionalComponentDefId, options ){
    ...
    entityFilters = entitySet.entityFilters;
    ...
    for( i=0,len=entityFilters.length;i<len;i++ ){
        if( !entityFilters[i].accept( entity, options) ){
            // if( options.debug ){
            //     log.debug(this.cid + ' failed: ' + result + ' (' + entityFilters[i] + ')');
            //     print_e(entity);
            // }
...
EntitySet.createCollection = function( entitySet, entityFilter, options ){
    ...
    result.entityFilters = entityFilter;
