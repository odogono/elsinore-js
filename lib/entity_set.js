...
var EntitySet = Backbone.Model.extend({
    ...
    setEntityFilter: function( entityFilter, options ){
        ...
        // log.debug('filter set ' + this.entityFilter);
    ...
    */
    isEntityOfInterest: function( entity, additionalComponentDefId, options ){
        if( this.entityFilter ){
            ...
            return this.entityFilter.accept( entity, options );
        }
    ...
    /*isComponentOfInterest: function( componentDef ){
        ...
        if( (include || optional) ){
            log.debug('include ' + componentDef.getName() + ' inc:' + inc + ' exc:' + exc + ' opt:' + opt);
            return true;
        }
        return false;
    },//*/
    ...
    */
    addComponent: function(component, options){
        ...
        if( !entity ){
            ...
            entity = this.addEntity( entity, ignoreComponentOptions );
        ...
        if( entity && !this.isEntityOfInterest( entity, componentDef.id ) ){
            // remove this entity in totality
            this.removeEntity( entity );
            return this;
        }

        return this._addComponent( entity, component, {} );
    ...
    _addComponent: function( entity, component, options ){
        var self = this;
        var silent, listenTo, componentDef, existingCom, existingEntity;
        ...
        if( !entity )
            entity = Entity.toEntity( component.getEntityId() );
        ...
        // log.debug('>adding ' + componentDef.getName() + ' to entity ' + JSON.stringify(entity) );
    ...
    */
    ...
    _removeComponent: function( entity, component, options ){
        var silent = options.silent;
    ...
    */
    addEntity: function( entity, options){
        var self = this, isNew, entityId, filteredEntity, existingEntity, debug, silent, ignoreComponents;
        var eBf, exBf, i, len, comDefId;
        ...
        if( this.entityFilter ){
            ...
            // if( filteredEntity ){
            //     log.debug('entity ' + entityId + ' failed filter');
            // }
        } else {
        ...
        if( !ignoreComponents ){
            ...
            if( !isNew ){
                // eBf = entity.getComponentBitfield().toValues();
                // exBf = existingEntity.getComponentBitfield().toValues();
                // for( i=0,len=exBf.length;i<len;i++ ){
                //     comDefId = exBf[i];
                //     if( _.indexOf( eBf, comDefId ) == -1 ){
                //         self._removeComponent( existingEntity, existingEntity.components[comDefId], options );
                //     }
                // }
    ...
    */
    ...
    getEntity: function( entity, options ){
        ...
        entityId = Entity.toEntityId(entity);// Utils.isInteger( entity ) ? entity : entity.id;
        existingEntity = this._entityObjs[ entityId ];
...
EntitySet.isEntitySet = function(es){
    return ( es && _.isObject(es) && es instanceof Entity );
}
...
EntitySet.create = function(options){
    ...
    // log.debug('YO ' + JSON.stringify(options) + ' ' + result.toString() );
    return result;
};
