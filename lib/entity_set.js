...
var EntitySet = Backbone.Model.extend({
    ...
    _reset: function( initialize ){
        ...
        this._cmdBuffer = new CmdBuffer();// {}; // a map of entityIds to arrays of commands TODO : replace with es6 Map
    },
    ...
    */
    onEntitySetEvent: function( evt ){
        ...
        // result.listenTo(self, 'entity:remove', result.remove );
        return this;
    },
    ...
    _executeCmdBuffer: function( cmdBuffer, options ){
        ...
        if( !silent ){
            if( cmdBuffer.componentsUpdated.length > 0 ){
                this.trigger('component:change', cmdBuffer.componentsUpdated );
            }

            if( cmdBuffer.componentsRemoved.length > 0 ){
                this.trigger('component:remove', cmdBuffer.componentsRemoved );
            }
            
            if( cmdBuffer.entitiesRemoved.length > 0 ){
                this.trigger('entity:remove', cmdBuffer.entitiesRemoved );
            }        

            if( cmdBuffer.componentsAdded.length > 0 ){
                this.trigger('component:add', cmdBuffer.componentsAdded );
            }

            if( cmdBuffer.entitiesAdded.length > 0 ){
                this.trigger('entity:add', cmdBuffer.entitiesAdded );
            }    
        }
...
CmdBuffer.prototype.reset = function(){
    ...
    this.componentsUpdated = Utils.clearArray( this.componentsUpdated );
}

/**
*   Adds a add/remove/update command to a buffer of commands
*/
CmdBuffer.prototype.addCommand = function( type, arg/*entityId|component*/, options ){
    var entityId;
    var entityBuffer;
...
CmdBuffer.prototype.findEntityAddId = function( cmdBuffer ){
    var cmds;
    var entityId;
...
var EntityCollection = Backbone.Collection.extend({
    ...
    onEvent: function(){
        ...
        // log.debug('collection ' + this.hash() + ' modified!');
    },
    ...
    */
    update: function(){
        ...
        // log.debug('added ' + added + '/' + models.length + ' entities to ' + this.hash());
    },
    ...
    hash: function(){
        ...
        if( this.entityFilter ){
            // log.debug('ef hash ' + this.entityFilter.hash() );
...
EntitySet.createCollection = function( entitySet, entityFilter, options ){
    var e, elen, i, len, entity, accept, hash;
