...
var EntitySet = Backbone.Model.extend({
    ...
    _reset: function(){
        ...
        this._componentDefs = BitField.create();
    ...
    setIncludedComponents: function( componentDefs, options ){
        ...
        for( var i=0;i<componentDefs.length;i++ ){
            ...
            // log.debug('include ' + componentDefId );
            this._includedComponents.set(  componentDefId, true );
        }

        // log.debug('included ' + this._includedComponents.count() );
    ...
    */
    isEntityOfInterest: function( entity ){
        ...
        // log.debug('iEoI ' + bf.toHexString() + ' ' + this._excludedComponents.toHexString() );
        if( this._excludedComponents.and( null, bf ) ){
            return false;
        }
        ...
        if( this._optionalComponents.and( null, bf ) ){
            return true;
        }

        if( this._includedComponents.and( null, bf ) ){
            return true;
        }
    ...
    addComponent: function(component, options){
        ...
        this.listenTo( component, 'change', function(){
            // log.debug('aha ' + JSON.stringify( _.toArray(arguments)));
