...
_.extend( EntitySetListener.prototype, {
    ...
    */
    listenTo: function(srcEntitySet, targetEntitySet, entityFilter, options){
        this.entitySet = srcEntitySet;
        this.targetEntitySet = targetEntitySet;
        this.entityFilter = entityFilter;

        srcEntitySet.listenTo( targetEntitySet, 'entity:add' );
        srcEntitySet.listenTo( targetEntitySet, 'entity:remove' );
        srcEntitySet.listenTo( targetEntitySet, 'component:add', _.bind(this.onComponentAdd,this) );
        srcEntitySet.listenTo( targetEntitySet, 'component:remove', _.bind(this.onComponentRemove,this) );
    },

    onComponentAdd: function(components){
        this.isModified = true;
        if( this.updateOnEvent ){
            this.entitySet.addComponent( components );
        }
    },
    ...
    onComponentRemove: function(components){
        ...
        // printIns( entitySet );
        // log.debug('removing components! ' + entitySet );

        // if none of these components are of interest, then no need to update
        if( this.updateOnEvent ){

            // filter out the components which belong to this entitySet
            components = _.filter( components, function(component){
                return entitySet.get( component.getEntityId() );
            });
            ...
            if( components.length > 0 ){
                entitySet.trigger('component:remove', components);
            }
        }
    },
    ...
    update: function(){
        ...
        // log.debug('adding ' + models.length + ' models');
        ...
        for( e=0,elen=models.length;e<elen;e++ ){
            if( EntitySet.isEntityOfInterest(this, models[e], entityFilter) ){
                entitySet.add( models[e] );
                added++;
            }
        }
    },
...
EntitySet.listenTo = function( srcEntitySet, targetEntitySet, entityFilter, options ){
    var updateOnEvent;
    ...
    listener.listenTo( srcEntitySet, targetEntitySet, entityFilter );
    listener.update();
...
EntitySet.createCollection = function( entitySet, entityFilter, options ){
    ...
    EntitySet.listenTo( result, entitySet, entityFilter, options );
