...
var EntitySet = Backbone.Model.extend({
    ...
    */
    ...
    _addComponent: function( entity, component, options ){
        ...
        existingCom = this._componentsById[ component.id ];

        if( existingCom ){
            existingCom.set( component.attributes );
            if( existingCom.hasChanged() ){
                this.trigger('component:change', existingCom, ComponentDef.getId( componentDef ), entity.id );
            }
            return this;
        }
    ...
    */
    addEntity: function( entity, options){
        var self = this, isNew, entityId, existingEntity, debug, silent, ignoreComponents;
        ...
        // already exist
        if( !ignoreComponents ){
            if( entity.components.length <= 0 || !this.isEntityOfInterest( entity ) ){
                if( existingEntity )
        ...
        if( !silent ){
            this.trigger('entity:add', existingEntity );
        }
