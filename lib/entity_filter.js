    ...
    */
    accept: function(entity, options){
        ...
        for( i=0,len=this._filters.length;i<len;i++ ){
            ...
            else if( !this.acceptEntity( entity, filter.type, filter.bf, filter.attrs, filter.comIId, null ) ){
                return false;
            }
        }
        return true;
    ...
    acceptEntity: function( entity, type, bitField, attrs, comIId, extra, debug ){
        var ebf, key, com;
        ...
        result._filters = _.map( filters, function(args){
            ...
            if( type === EntityFilter.ATTRIBUTES ){
                if( _.isObject(args[args.length-1]) ){
                    ...
                    result.comIId = args[0]; // attributes only operate on a single component
                }
            }
