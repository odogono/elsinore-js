...
_.extend( EntityFilter.prototype, {
    add: function( type, bitField ){
        this.filters.push( { type:type, bitField:bitField } );
    },

    getValues: function(index){
        var filter = this.filters[index];
        return filter.bitField.toValues();
    },

    accept: function( entity, options ){
        var ii,len, filter;
        var ebf = entity.getComponentBitfield();
        for( ii=0,len=this.filters.length;ii<len;ii++ ){
    ...
    hash: function( asString ){
        return Utils.hash( 'EntityFilter' + JSON.stringify(this.filters), asString );
    }
...
EntityFilter.combine = function( filters ){
    ...
    var result = EntityFilter.create(); //new ComponentFilter();
