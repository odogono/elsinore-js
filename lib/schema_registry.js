...
Schema.create = function( obj, uri, hash ){
    var result = Object.create( Schema );
...
_.extend(SchemaRegistry.prototype, Backbone.Events, {
    ...
    register: function( schema, options ){
        ...
        for( i=0,len=objs.length;i<len;i++ ){
            ...
            if( this.schemasByHash[entry.hash] ){
                throw new Error('schema ' + entry.uri + ' already exists' );
            }
    ...
    *   internal ids
    */
    getIId: function( schemaIdentifiers ){
        var i;
        var len;
        var iid, id, val;
        ...
        for( i=0,len=args.length;i<len;i++ ){
            id = args[i];
            iid = undefined;
            if( (val = this.schemasByHash[id]) ){
                iid = val.iid; //this.schemaIds.indexOf(val);
                // result.push( this.schemaIds )
            } else if( (val = this.schemasByUri[id]) ){
                iid = val.iid;
            } else if( (val = this.schemaIds[ id ]) ){
                iid = id;
            }
            result.push( iid );
        }
