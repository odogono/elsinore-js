...
_.extend(SchemaRegistry.prototype, Backbone.Events, {
    ...
    */
    register: function( schema, options ){
        ...
        for( i=0,len=objs.length;i<len;i++ ){
            ...
            if( this.versions[uri] && this.schemaByHash[uri][hash] ){
        ...
        // objs = _.map( objs, function(entry){
        //     return replaceSubObjects( self.schemasByUri, entry.obj, entry.uri );
        // });
        ...
        for( i=0,len=objs.length;i<len;i++ ){
            ...
            console.log('replacing ' + entry.uri + ' ' + subObjs.hash + ' ' + entry.hash );

            delete this.versions[ uri ][ entry.hash ];
            delete this.schemaByHash[ entry.hash ];
            this.schemaByHash[ subObjs.hash ] = subObjs;
            this.schemasByUri[ subObjs.uri ] = subObjs;
            this.versions[ uri ][ subObjs.hash ] = subObjs;
        }
    ...
    */
    get: function( schemaId, schemaHash, options ){
        ...
        returnFull = options && options.full;

        // atempt to return by hash
        result = this.schemaByHash[schemaId];

        if( !result && schemaHash ){
            result = this.versions[schemaId][schemaHash];
        }
        ...
        if( !result ){
            ...
            result = this.schemasByUri[ uri.baseUri ].obj;
            isRootSchema = false;
