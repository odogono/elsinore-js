...
BitField.prototype.equals = function(other){
    ...
    for (ii=0,len=this.values.length;ii<len; ii++) {
        a = this.values[ii];
        b = other.values[ii];
        a = a === undefined ? 0 : a;
        b = b === undefined ? 0 : b;
...
BitField.prototype.set = function(i, value) {
    ...
    this.all = false;
    if( BitField.isBitField(i) ){
        return this.setValues( i.toValues(), true );
    }
    index = (i / 32) | 0;
    bit = i % 32;
    if( value ){
...
BitField.prototype.toValues = function( values ){
    var ii,len,result = [];
    values = (values || this.values);
    for( ii=0,len=(values.length*32);ii<len;ii++ ){
...
BitField.prototype.toArray = function ( values ){
    ...
    result = [];
    values = values || this.values;
    for (ii=(values.length*32)-1;ii>=0; ii--) {
        v = this.get(ii, values);
        if( !found ){
            found = v;
        }
        if( found ){
...
BitField.isBitField = function( val ){
    return ( val && val instanceof BitField );
};
...
BitField.create = function( values ){
    ...
    result = new BitField();
    if( values ){
        if( BitField.isBitField(values) ){
            values = values.toString();
        }
        if( _.isArray(values) ){
            result.setValues( values, true );
        }
